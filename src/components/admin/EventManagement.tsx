...
const EventManagement: React.FC<EventManagementProps> = ({ events, users, onRefresh }) => {
  ...
  const handleUpdateEvent = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { error } = await db.updateEvent(selectedEvent.id, editFormData);
      if (error) {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsLoading(true);
    try {
      const { error } = await db.deleteEvent(selectedEvent.id);
      if (error) {
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6">
            <div className="space-y-6">
              <div>
                <h4 className="text-lg font-semibold text-gray-900 mb-2">{selectedEvent.event_name}</h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-gray-600">Expected Attendees</span>
                  </div>
                  <p className="text-lg font-semibold text-gray-900">{selectedEvent.expected_attendees}</p>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="flex items-center space-x-2 mb-2">
              ...
              {selectedEvent.location_address && (
                <div>
                  ...
                  <p className="text-gray-900">{selectedEvent.location_address}</p>
                </div>
              )}
              ...
              <div>
                <h5 className="text-sm font-medium text-gray-600 mb-2">Organized by</h5>
                <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center">
                    <Users className="w-4 h-4 text-indigo-600" />
                  </div>
                  <div>
                    <p className="font-medium text-gray-900">{getUserName(selectedEvent.user_id)}</p>
                    <p className="text-sm text-gray-500">{getUserEmail(selectedEvent.user_id)}</p>
                  </div>
                </div>
              </div>

              <div className="text-sm text-gray-500">
                <p>Created: {new Date(selectedEvent.created_at).toLocaleString()}</p>
                <p>Updated: {new Date(selectedEvent.updated_at).toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <tr key={event.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{getUserEmail(event.user_id)}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
