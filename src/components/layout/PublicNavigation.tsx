...
const PublicNavigation: React.FC = () => {
  ...
  const [isAuthModalOpen, setIsAuthModalOpen] =import React, { useState, useEffect } from 'react';
import { Menu, X, Calendar, Users, Building, BookOpen, Info, Phone, User } from 'lucide-react';
import { useApp } from '../../contexts/AppContext';
import { useAuth } from '../../contexts/AuthContext';
import AuthModal from '../auth/AuthModal';
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-indigo-600 to-purple-600 backdrop-blur-md bg-opacity-90 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className={`md:hidden transition-all duration-300 ease-in-out ${
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white rounded-lg shadow-xl mt-2">
              {navigationItems.map((item) => {
                const IconComponent = item.icon;
                return (
                  <button
              ...
              <div className="border-t border-gray-200 pt-3 mt-3">
                <button
                  onClick={() => handleNavigation('pricing')}
                  className="text-gray-700 hover:text-indigo-600 block px-3 py-2 text-base font-medium w-full text-left rounded-lg hover:bg-indigo-50 transition-colors duration-200"
                >
                  Pricing
                </button>
                <button
...
export default PublicNavigation; useState(false);
  const { setCurrentView } = useApp();
  const { isAuthenticated, user, logout } = useAuth();
