...
import { OrganizerEvent, Attendee } from '../../types/organizerEvent';
...
const AttendeeManagementPage: React.FC = () => {
  ...
  const loadEvents = async () => {
    try {
      ...
      setEvents(eventsData.filter(e => e.status !== 'draft')); // Only show published events
  ...
  const loadAttendees = async (eventId: string) => {
    try {
      const attendeesData = await organizerEventService.getEventAttendees(eventId);
  ...
  const handleStatusUpdate = async (attendeeId: string, status: 'pending' | 'checked-in' | 'no-show') => {
    try {
      const result = await organizerEventService.updateAttendeeStatus(selectedEvent, attendeeId, status);
      if (result.success) {
  ...
  const handleExportAttendees = () => {
    const csvContent = [
      ['Name', 'Email', 'Ticket Type', 'Registration Date', 'Check-in Status', 'Payment Status'],
    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Attendee Management</h1>
        ...
        {isLoading ? (
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredAttendees.map((attendee, index) => (
                        <tr key={attendee.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center space-x-2">
                              {getPaymentIcon(attendee.paymentStatus)}
                              <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
                                attendee.paymentStatus === 'completed' ? 'bg-green-100 text-green-800' :
                                attendee.paymentStatus === 'refunded' ? 'bg-red-100 text-red-800' :
          ...
          <div className="text-center py-20">
            <div className="bg-white rounded-2xl p-12 shadow-lg">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No published events</h3>
