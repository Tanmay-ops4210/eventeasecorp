...
export const eventService = {
  ...
  async createEvent(eventData: Partial<Event>): Promise<{ ok: boolean, message: string, data?: Event }> {
    ...
    if (currentUser.plan === 'FREE' && myEvents.filter(e => e.status === 'PUBLISHED' && e.ownerId === currentUser._id).length >= constraints.maxActiveEvents) {
  ...
  async publishEvent(eventId: string): Promise<{ ok: boolean, message: string }> {
     ...
     // Validation Checks
     if (!event.waterfall.requirements.title || !event.waterfall.implementation.capacity) {
         return { ok: false, message: 'Title and Capacity are required to publish.'};
     }
     if (event.planSnapshot.tier === 'FREE') {
         if (event.waterfall.implementation.capacity > constraints.maxCapacity) {
